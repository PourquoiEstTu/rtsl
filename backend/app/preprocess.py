...
#   later
def hand_keypoint_to_img(keypoint_file: str, img_size: int = 200) :
    ...
    keypoint_vid = np.load(keypoint_file) * img_size 
    keypoint_vid = keypoint_vid.astype(np.int32)
    # print(keypoint_vid))
    # print(np.max(keypoint_vid))
    # return
    n_frames = keypoint_vid.shape[0]
    keypoints_per_frame = keypoint_vid.shape[1]

    # divisible by number should be changed when pose and face keypoints are added
    if keypoints_per_frame % 3 != 0 : 
        raise Exception("Number of keypoints not divisible by 3")
    ...
    imgs = []
    for frame in keypoint_vid :
        # don't start patience counter until non-zero frame is encountered
        if np.any(frame) :
            real_frame_encountered = True
            # print("entered!")
        if real_frame_encountered and not np.any(frame) : 
            patience += 1
        else :
            patience = 0
        if patience >= 3 :
        ...
        for x_coord in range(0, keypoints_per_frame - 3, 3) :
            y_coord = x_coord+1
            if frame[y_coord] >= img_size : 
                frame[y_coord] = img_size - 1
            if frame[x_coord] >= img_size : 
                frame[x_coord] = img_size - 1
            img[frame[y_coord]][frame[x_coord]] = 1
        imgs.append(img)
