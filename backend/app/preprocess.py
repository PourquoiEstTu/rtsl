...
#   later
def hand_keypoint_to_img(keypoint_file: str) :
    """Takes a set of keypoints (single file)  generated by Mediapipe and 
       converts them into a 224x224  image that can be inputted to ResNet
    """
    keypoints = np.load(keypoint_file)*224 # keypoints are all between 0 and 1
    img = np.zeros((224,224,11))
    cv2.imshow("", img)
hand_keypoint_to_img(f"{TRAIN_OUTPUT_DIR_NORMALIZED}/69541.npy")
